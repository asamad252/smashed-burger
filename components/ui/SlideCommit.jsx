...
export default function SlideCommit({
  ...
  useEffect(() => {
    ...
    if (!node) return;
    const measure = () => setMeasuredWidth(node.clientWidth || width);
    measure();
  ...
  const INNER = measuredWidth - PAD * 2;
  ...
  const local = clientX => {
    ...
    if (!rect) return 0;
    return (clientX - rect.left) / (rect.width / measuredWidth || 1);
  };
  ...
  const up = e => {
    ...
    setHeld(false);
    if (e.type !== 'pointercancel' && e.type !== 'cancel' && x.get() >= TRAVEL) commit(false);
  ...
  const onKeyDown = e => {
    ...
    if (e.key === 'End' || e.key === 'Enter' || e.key === ' ') {
      ...
      e.preventDefault();
      if (grip.current) up({ pointerId: grip.current.id, type: 'cancel' });
      else x.set(0);
    }
  };
